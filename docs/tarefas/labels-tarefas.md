...
| **REQ/ADR/DT** | Referências relacionadas a requisitos, decisões arquiteturais e dívidas técnicas.<br><br>Tipos aceitos:<br>`RF` = requisito funcional<br>`RN` = regra de negócio<br>`RNF` = requisito não-funcional<br>`ADR` = Architecture Decision Record<br>`DT` = dívida técnica<br><br>Use `-` se não houver referência.<br><br>Exemplo:<br>`RF-2, ADR-3, DT-14` |
...
| TAS-RNF  | Requisito Não-Funcional      |
| TASK-FBG | Bug                          |
| TASK-REF | Refatoração                  |
| TASK-DOC | Documentação                 |
| TASK-DT  | Dívida Técnica               |
| TASK-REV | Revisão                      |
