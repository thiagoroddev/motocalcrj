...
- Cada tarefa tem: REQ (referencia do requisito), Acao, Criterio, Status e Observacoes.
- Atualize o Status para [x] ao concluir.
- Use Observacoes quando houver mudanca de requisito ou ajuste de escopo.
- As tarefas fazem referencia aos requisitos do documento docs/Requisitos_MotoCalc_RJ_v6.md.
- Consulte outros documentos para melhor contexto:
  - docs/arquivo-antigos-apenas-consulta/Setup.md
  - docs/arquivo-antigos-apenas-consulta/rules-ai.md
  - docs/arquivo-antigos-apenas-consulta/Convencoes.md
  - docs/Arquitetura_Funcoes_Calculo.md
  - docs/ESTADO_INICIAL.md
  - docs/Design_MotoCalc_Figma.md
  - docs/mapeamento-ui-figma/

## Politica de testes (Vitest)

- **Marco de testes (expandido):** a partir das tasks que mexem em **estado/persistencia**, **calculos**, **registros**, **overrides**, **import/export**, **migracoes** e **analytics**.
- Para essas tasks, **rodar testes ao finalizar** e **so seguir se estiverem verdes**.
- Mudancas apenas de UI/UX nao exigem testes, desde que nao alterem logica, estado ou calculos.
- Comando padrao: `npm run test` (ou `vitest run`).
- **Cobertura atual no backlog:** TASK-1.1 a TASK-1.3, TASK-3.1 a TASK-3.4, TASK-5.1 a TASK-5.5, TASK-6.1 a TASK-6.3, TASK-7.1, TASK-7.2, TASK-8.1.

## Mapa de execucao (ordem sugerida)

1. Base do projeto, tema e dados
2. Estado global, persistencia e rotas
3. Onboarding completo + FIPE
4. Calculos core + testes
5. Estimativa e detalhamento
6. Registros e formularios
7. Mao de obra, autonomia, perfil
8. Export/import e presets
9. Analytics, PWA, TWA
10. QA, performance, acessibilidade
