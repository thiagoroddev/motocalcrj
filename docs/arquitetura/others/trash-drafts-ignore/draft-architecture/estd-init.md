...
## I- Multi-Perfil: Cada Usuário Pode Ter N Predefinições

O app suporta múltiplos perfis salvos localmente (RF-PERF-02). Um motoboy pode ter "Trabalho" e "Pessoal", ou comparar dois modelos de moto. A estrutura no localStorage reflete isso.

### I.1- Chaves do localStorage

| Chave | Tipo | Descrição |
|---|---|---|
| `motocalc:v5:perfis` | `PerfilEntry[]` (JSON) | Array com todos os perfis salvos. Máximo recomendado: 5 |
| `motocalc:v5:perfilAtivo` | `string` | `perfilId` do perfil atualmente ativo |

> **Convenção de namespace:** `motocalc:v5:` para isolar dados e facilitar migrations. A versão (`v5`) refere-se ao `schemaVersion`. Ao migrar para v6, o hook `migrarPerfil.ts` lê `v5:perfis`, transforma, e grava em `v6:perfis`.

### I.2- Tipo PerfilEntry (wrapper de perfil para multi-perfil)

```typescript
// Não está em PerfilUsuario — é o envelope do localStorage
interface PerfilEntry {
  perfilId: string           // nanoid() — gerado na criação
...
App.tsx renderiza
       ↓
usePerfil.ts → localStorage.getItem('motocalc:v5:perfilAtivo')
       ↓
  null?  →  nenhum perfil ativo → redirecionar para /onboarding
  string → perfilId encontrado
       ↓
usePerfil.ts → localStorage.getItem('motocalc:v5:perfis')
       ↓
  null?  →  array vazio → redirecionar para /onboarding
  array  → procurar entrada com perfilId === ativo
       ↓
  não encontrado? → usar primeiro perfil do array
  encontrado      → carregar perfil no PerfilContext via dispatch CARREGAR_PERFIL
       ↓
...
type PerfilAction =
  ...
  // ── Overrides de Peças ──────────────────────────────────────────
  | { type: 'SET_PECA_OVERRIDE';   id: string; campo: 'preco' | 'intervaloKm' | 'perfilPecas'; valor: number | PerfilPecas }
  ...
  // Chamado ao abrir o app — hidrata o estado com o perfil salvo

  | { type: 'RESETAR_PERFIL' }
  // Volta para perfilPadrao e apaga o perfil do localStorage
...
### V.2- ADD_* de registros (atualização automática de kmAtual)
...
interface IPerfilStorage {
  carregarTodos(): PerfilEntry[]
  salvarTodos(perfis: PerfilEntry[]): void
  getAtivo(): string | null
  setAtivo(perfilId: string): void
  limpar(): void
}

class LocalStoragePerfilStorage implements IPerfilStorage {
  private readonly CHAVE_PERFIS  = 'motocalc:v5:perfis'
  private readonly CHAVE_ATIVO   = 'motocalc:v5:perfilAtivo'

  carregarTodos(): PerfilEntry[] {
    try {
      const raw = localStorage.getItem(this.CHAVE_PERFIS)
  ...
  salvarTodos(perfis: PerfilEntry[]): void {
    localStorage.setItem(this.CHAVE_PERFIS, JSON.stringify(perfis))
  }
  ...
  setAtivo(perfilId: string): void {
    localStorage.setItem(this.CHAVE_ATIVO, perfilId)
  }

  limpar(): void {
    localStorage.removeItem(this.CHAVE_PERFIS)
...
if (import.meta.env.DEV && !localStorage.getItem('motocalc:v5:perfis')) {
  const fixture = await import('../presets/usuario_teste.json')
  localStorage.setItem('motocalc:v5:perfis',      JSON.stringify(fixture['motocalc:v5:perfis']))
  localStorage.setItem('motocalc:v5:perfilAtivo', fixture['motocalc:v5:perfilAtivo'])
  console.info('[DEV] Fixture carregado: João da Silva — 2 perfis')
}
```
...
*Próxima etapa: Fase 1 — Setup do projeto (npm create vite@latest).*
